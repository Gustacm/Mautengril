...
getUserLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      // -35.42772301473541, -71.6563411

      //saber a cuantos esta el usuario de la tienda
      const distance = this.getDistanceFromLatLonInMeters(latitude, longitude, latitudeTienda, longitudeTienda);
      //saber si esta a menos de 1 km de la tienda
      this.isNear(latitude, longitude, latitudeTienda, longitudeTienda);

      console.log(`Latitude: ${latitude}, Longitude: ${longitude}, Distance: ${distance} m`);
    });
  } else {
...
getDistanceFromLatLonInMeters(lat1:any, lon1:any, lat2:any, lon2:any) {
  ...
  return Math.round(d * 1000); // Convert to meters
}
